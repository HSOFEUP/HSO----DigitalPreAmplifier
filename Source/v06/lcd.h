...
void LCD_sendChar(uint8_t);		//forms data ready to send to 74HC164
void LCD_sendCommand(uint8_t);	//forms data ready to send to 74HC164
void LCD_init(void);			//Initializes LCD
void LCD_clr(void);				//Clears LCD
void LCD_home(void);			//LCD cursor home
void LCD_sendString(char *data);	//Outputs string to LCD
void LCD_gotoXY(uint8_t, uint8_t);	//Cursor to X Y position
void Copy_StringtoLCD(const uint8_t*, uint8_t, uint8_t);//copies flash string to LCD at x,y
void LCD_defineChar(const uint8_t *,uint8_t);//write char to LCD CGRAM 
void LCD_shiftRight(uint8_t);	//shift by n characters Right
void LCD_shiftLeft(uint8_t);	//shift by n characters Left
void LCD_setCursorOn(void);		//Underline cursor ON
void LCD_setCursorOnBlink(void);	//Underline blinking cursor ON
void LCD_setCursorOFF(void);		//Cursor OFF
void LCD_setBlank(void);			//LCD blank but not cleared
void LCD_setVisible(void);			//LCD visible
void LCD_cursorLeft(uint8_t);	//Shift cursor left by n
void LCD_cursorRight(uint8_t);	//shif cursor right by n
