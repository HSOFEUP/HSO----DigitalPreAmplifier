...
/* @initSHT25                                                           */
/************************************************************************/
void initSHT25(uint8_t addr, uint8_t mode){			//??MODE?? HOLD/NO-HOLD? MAS ISSO É O COMANDO!?!?
//aqui e preciso mais uma coisas

	I2C_start();
	I2C_sendAddr(addr);
	I2C_sendData(SHT25_WRITE_USER_REGISTER);
	I2C_sendData(mode);
	I2C_stop();
}
...
uint8_t data;
	...
	I2C_start();
	I2C_sendAddr(addr);
	I2C_sendData(SHT25_READ_USER_REGISTER);
	I2C_repeatStart();
	I2C_sendAddr(addr+1);
...
uint16_t readTempSHT25(uint8_t addr, uint8_t cmd){
	uint16_t data;
		...
		data = (uint16_t) (I2C_receiveData_ACK()<<8);	//RECEIVE MSB FIRST
		data |= I2C_receiveData_NACK();					//THEN RECEIVE LSB
		I2C_stop();
...
uint16_t calcTempSHT25(uint8_t addr, uint8_t cmd){
	...
	float tempValue = 0;								
		...
		sigTemp = readTempSHT25(addr,cmd);
...
uint16_t readRHSHT25(uint8_t addr, uint8_t cmd){
	uint16_t data;
		...
		data = (uint16_t) (I2C_receiveData_ACK()<<8);	//RECEIVE MSB FIRST
		data = (uint8_t) (I2C_receiveData_NACK());		//THEN RECEIVE LSB
		I2C_stop();
		return data;								
}
...
/************************************************************************/
float calcRHAbsoluteSHT25(uint16_t temperature, uint16_t rh){
